...
`tea-code` 是 Tea Code CLI 的主命令，用于在终端中启动一次交互式会话。不带任何参数运行时，它会在当前工作目录下开启一个新会话；配合不同的 flag，可以续上历史会话、跳过审批、从 Plan 模式开始，或者指定自定义的 Skills 目录。
...
恢复会话时，可以通过 `--auto`、`--yolo` 或 `--plan` 覆盖原会话保存的权限或计划模式。例如，`tea-code --continue --auto` 会恢复最近会话并切换到 "Never Ask" 模式。
...
把 Tea Code CLI 切换到 ACP（Agent Client Protocol）模式，在标准输入/输出上以 JSON-RPC 形式与 IDE 对话，让编辑器直接驱动 kimi 的会话和工具调用。通常不需要手动运行——IDE 会把它作为子进程入口启动。配置方式见[在 IDE 中使用](../guides/ides.md)，技术细节见 [tea-code acp 参考](./kimi-acp.md)。
...
::: info 提示
`tea-code server` 命令树已废弃：任何 `tea-code server …` 调用（含全部旧子命令）只会打印弃用提示并以退出码 1 结束，请改用 `tea-code web`。唯一的例外是 `tea-code server kill`，它仍然可用，仅用于停止 0.28.0 之前版本启动的服务。该提示将在 Kimi Code 下个大版本移除。
:::
...
已废弃——仅用于停止 0.28.0 之前的 Kimi Code 版本启动的服务。那些版本可能在后台遗留服务进程，记录在 legacy 单实例锁文件 `~/.tea-code/server/lock` 中；该命令先请求 `POST /api/v1/shutdown` 优雅退出，再对锁中记录的 pid 发 SIGTERM、必要时升级为 SIGKILL，并在确认进程退出后删除锁文件。`tea-code web` 启动的服务在前台运行，直接用 `Ctrl+C` 停止即可。
...
对全局 npm、pnpm、yarn、bun 安装，`tea-code upgrade` 会展示更新选项；选择 `Install update now` 后运行对应的前台安装命令。对本地构建的 native 二进制和 Homebrew 安装布局，会打印 Tea npm 安装命令，不自动替换这些构建。当前安装方式无法自动升级时，改为打印手动更新命令。
