...
`tea-code acp` 把 Tea Code 切换到 **ACP (Agent Client Protocol)** 模式：在标准输入/输出上以 JSON-RPC 形式与 ACP 客户端（如 Zed、JetBrains AI Chat 等）对话，让 IDE 直接驱动 tea-code 的会话、prompt 与工具调用。
...
| 方法 | 状态 | 说明 |
| --- | --- | --- |
| `initialize` | 是 | 版本协商；返回 `agentInfo: { name: 'Tea Code CLI', version }`、能力矩阵、`authMethods` |
...
- `http` → `kimi` 供应商的 `transport: 'http'` 配置
- `stdio` → `kimi` 供应商的 `transport: 'stdio'` 配置
- `sse` → `kimi` 供应商的 `transport: 'sse'` 配置
- `acp` → 丢弃并写一条 warn 日志
