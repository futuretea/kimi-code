...
The local server started by `tea-code web` exposes two programmatic surfaces: a REST API (`/api/v1`, plus `/api/v2/sessions`) and a WebSocket event stream (`/api/v1/ws`). This page is the protocol reference for both. For how to start the server and its command-line options, see the [tea-code command](./kimi-command.md#tea-code-web) reference; for an end-to-end walkthrough, see [Local server and API](../guides/server.md).
