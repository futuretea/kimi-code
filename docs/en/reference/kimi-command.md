...
When resuming a session, you can override its saved permission or plan mode by adding `--auto`, `--yolo`, or `--plan`. For example, `tea-code --continue --auto` resumes the latest session and switches it to Never Ask mode.
...
Switch Tea Code CLI to ACP (Agent Client Protocol) mode, communicating with an IDE via JSON-RPC over stdin/stdout so the editor can directly drive kimi's sessions and tool calls. You typically do not need to run this manually — the IDE starts it as a subprocess entry point. For configuration, see [Using in IDEs](../guides/ides.md); for technical details, see the [tea-code acp reference](./kimi-acp.md).
...
For global npm, pnpm, yarn, and bun installations, `tea-code upgrade` shows update options; selecting `Install update now` runs the corresponding foreground install command. For locally built native binaries and Homebrew layouts, it prints a Tea npm installation command; these builds are not updated automatically. When the current installation method cannot be upgraded automatically, the manual update command is printed instead.
