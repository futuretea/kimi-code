...
When resuming a session, you can override its saved permission or plan mode by adding `--auto`, `--yolo`, or `--plan`. For example, `tea-code --continue --auto` resumes the latest session and switches it to auto permission mode.
...
- **`--skills-dir <dir>`** (CLI flag): **Replaces** the automatically discovered user and project directories for this launch only. Can be repeated to stack multiple directories:
  ...
  ```sh
  tea-code --skills-dir /path/to/team-skills --skills-dir ./local-skills
  ```
...
`--agent` and `--agent-file` select which agent drives a new session, in both print mode (`tea-code -p`) and the interactive TUI:
...
```sh
tea-code -p "Summarize the current repository status"
```
...
```sh
tea-code -m kimi-code/kimi-for-coding -p "Explain the latest diff"
```

When you need to parse output programmatically, use the `stream-json` format — each line on stdout is a JSON object:

```sh
tea-code -p "List changed files" --output-format stream-json
```
...
Switch Tea Code to ACP (Agent Client Protocol) mode, communicating with an IDE via JSON-RPC over stdin/stdout so the editor can directly drive tea-code's sessions and tool calls. You typically do not need to run this manually — the IDE starts it as a subprocess entry point. For configuration, see [Using in IDEs](../guides/ides.md); for technical details, see the [tea-code acp reference](./kimi-acp.md).
...
Run the local Tea Code server in the foreground of the current terminal — a single process that exposes the REST + WebSocket API and serves the web UI from the same origin — and open the web UI in the default browser once it is ready. The command stays attached to the terminal and shuts down cleanly on `SIGINT` / `SIGTERM` (e.g. `Ctrl-C`).
...
tea-code web --no-open       # don't open the browser
tea-code web --port 58628    # pick a specific bind port
```

Multiple instances can share one home directory: each registers itself under `~/.tea-code/server/instances/`, and a busy port is retried with `port + 1` (58628, 58629, …).
...
`tea-code web` binds to local loopback only by default and prints the bearer token in the startup banner; the web UI authenticates automatically via the `#token=` URL fragment.

::: info
The `tea-code server` command tree is deprecated: any `tea-code server …` invocation (including all legacy subcommands) only prints a deprecation notice and exits with code 1 — use `tea-code web` instead. The one exception is `tea-code server kill`, which stays functional for stopping servers started by a version before 0.28.0. The notice will be removed in the next major version of Tea Code.
:::
...
#### `tea-code server kill`

Deprecated — only stops a server started by a version before 0.28.0. Those versions could leave a background server behind, recorded in the legacy single-instance lock at `~/.tea-code/server/lock`; the command first tries `POST /api/v1/shutdown` for a graceful exit, then signals the recorded pid with SIGTERM, escalating to SIGKILL when needed, and removes the lock file once the process is confirmed dead. Servers started by `tea-code web` run in the foreground — stop them with `Ctrl+C` instead.

#### `tea-code web rotate-token`

Generate a new persistent bearer token (written to `~/.tea-code/server.token`); the previous token stops working immediately. The token is shared by the whole home directory, so every running instance picks the new one up on its next auth check — no restart needed.

### `tea-code doctor`

Validate `config.toml` and `tui.toml` without starting the TUI or modifying either file. By default, the command checks the files under `TEA_CODE_HOME` (or `~/.tea-code` when the environment variable is unset). Missing default files are reported as skipped because built-in defaults can apply.
...
| Command | Description |
| --- | --- |
| `tea-code doctor` | Validate the default `config.toml` and `tui.toml` |
| `tea-code doctor config [path]` | Validate only `config.toml`, using `path` instead of the default file when provided |
| `tea-code doctor tui [path]` | Validate only `tui.toml`, using `path` instead of the default file when provided |
...
```sh
tea-code export [sessionId] [options]
```
...
The export contains all files in the target session directory. The global diagnostic log (`~/.tea-code/logs/kimi-code.log`) is included by default because it may contain events from other sessions or projects; add `--no-include-global-log` if you do not want to share it.
...
tea-code export 01HZ...XYZ -o ./bug-report.zip

# Exclude the global diagnostic log
tea-code export 01HZ...XYZ -o ./bug-report.zip --no-include-global-log
```

### `tea-code migrate`

Migrate local data from a legacy Kimi CLI installation to Tea Code, including session history and configuration files. Runs entirely interactively, guiding you through the full process.
...
Immediately check for the latest version and display an update prompt; exits after you make a selection. `tea-code update` is an alias for this command.

```sh
tea-code upgrade
```

For global npm, pnpm, yarn, and bun installations, `tea-code upgrade` shows update options; selecting `Install update now` runs the corresponding foreground install command. Native and other unsupported installations print the npm command instead; Tea Code never downloads a native update from the Kimi CDN.
...
```sh
tea-code vis [sessionId] [options]
```
...
```sh
tea-code provider <action> [options]
```

Five actions are available:

#### `tea-code provider add <url>`
...
```sh
tea-code provider add https://registry.example.com/v1/models/api.json --api-key YOUR_KEY

# Or via environment variable (suitable for CI / .envrc)
KIMI_REGISTRY_API_KEY=YOUR_KEY tea-code provider add https://registry.example.com/v1/models/api.json
```
...
#### `tea-code provider remove <providerId>`
...
```sh
tea-code provider remove kohub
```

#### `tea-code provider list`
...
```sh
tea-code provider list
tea-code provider list --json | jq '.providers | keys'
```

#### `tea-code provider catalog list [providerId]`
...
```sh
tea-code provider catalog list
tea-code provider catalog list --filter anthropic
tea-code provider catalog list anthropic
```

#### `tea-code provider catalog add <providerId>`
...
```sh
tea-code provider catalog list anthropic          # Browse available models first
tea-code provider catalog add anthropic --api-key sk-ant-... --default-model claude-opus-4-7
```
