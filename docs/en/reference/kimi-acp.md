...
`tea-code acp` switches Tea Code to **ACP (Agent Client Protocol)** mode: it communicates with an ACP client (such as Zed, JetBrains AI Chat, etc.) via JSON-RPC over stdin/stdout, letting the IDE directly drive tea-code's sessions, prompts, and tool calls.
...
| --- | --- | --- |
| `initialize` | Yes | Version negotiation; returns `agentInfo: { name: 'Tea Code CLI', version }`, capability matrix, and `authMethods` |
...
- `http` → the `kimi` provider's `transport: 'http'` configuration
- `stdio` → the `kimi` provider's `transport: 'stdio'` configuration
- `sse` → the `kimi` provider's `transport: 'sse'` configuration
