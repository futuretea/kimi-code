...
Hooks are an automatic trigger mechanism: you tell Tea Code in advance "whenever X happens, run this script." The script runs on your local machine, and you can put any logic inside it. Typical use cases:
...
After blocking, Tea Code writes the blocking reason back into the context, and the model can use this to choose a safer alternative.
