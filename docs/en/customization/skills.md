...
Agent Skills are a lightweight mechanism for extending model capabilities in Tea Code. A Skill is a Markdown document with YAML frontmatter that describes a specialized area of knowledge or a workflow — for example, a project's code style guidelines, a PR review process, or a commit message format.
...
Tea Code scans four tiers by scope; more specific scopes take higher priority: **Project > User > Extra > Built-in**

**User level** (applies to all projects):
- `$TEA_CODE_HOME/skills/` (default: `~/.tea-code/skills/`)
- `~/.agents/skills/`

The Tea Code-specific user Skill directory moves with `TEA_CODE_HOME`, so isolated data roots also get isolated Tea Code-specific Skills. The generic `~/.agents/skills/` directory stays under the real OS home so it can be shared across tools.
...
Save this as `$TEA_CODE_HOME/skills/review-pr/SKILL.md` (or `~/.tea-code/skills/review-pr/SKILL.md` when `TEA_CODE_HOME` is unset), place the checklist at `references/checklist.md` in the same directory, and after starting a new session you can invoke it with `/skill:review-pr #1234`, where `#1234` is expanded into `$pr_ref`.
