...
Tea Code CLI stores the config file, session history, login credentials, diagnostic logs, and other runtime data under `~/.tea-code/`. This page helps you understand where each type of data lives, what it is for, and how to clean up or relocate it when needed.
...
```sh
export TEA_CODE_HOME="$HOME/.config/kimi-code"
```

Once set, **all** Kimi Code data lands under the new path: config, sessions, logs, OAuth credentials, Kimi-specific user Skills, global `AGENTS.md`, and more. For the full reference on `TEA_CODE_HOME`, see [Environment variables](./env-vars.md).

::: tip Note

**Generic `.agents` resources** stay under the real OS home so they can be shared across tools. For example, user-level generic Skills remain at `~/.agents/skills/`, while Kimi-specific user Skills move with `TEA_CODE_HOME` as `$TEA_CODE_HOME/skills/`.
:::
...
- **`AGENTS.md`**: global Kimi-specific agent instructions. This file moves with `TEA_CODE_HOME`; generic cross-tool instructions can still live under `~/.agents/AGENTS.md`.
...
- **`skills/`**: Kimi-specific user-level Skills. This directory moves with `TEA_CODE_HOME`; generic cross-tool Skills can still live under `~/.agents/skills/`. See [Agent Skills](../customization/skills.md).
...
| Clear global Kimi-specific agent instructions | Delete `$TEA_CODE_HOME/AGENTS.md` (default `~/.tea-code/AGENTS.md`) |
| Clear plugin install records | Delete `$TEA_CODE_HOME/plugins/` (local plugin source directories are not affected) |
| Clear Kimi-specific user-level Skills | Delete `$TEA_CODE_HOME/skills/` (default `~/.tea-code/skills/`) |
