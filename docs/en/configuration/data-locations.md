...
Tea Code stores all runtime data — the config file, session history, login credentials, and diagnostic logs — under `~/.tea-code/`. This page helps you understand where each type of data lives, what it is for, and how to clean up or relocate it when needed.

## Data root directory

The default data root is `~/.tea-code/`. The actual path varies by platform:

- macOS: `/Users/<name>/.tea-code`
- Linux: `/home/<name>/.tea-code`
- Windows: `C:\Users\<name>\.tea-code`

If you need to move the data directory elsewhere (for example, to isolate configs for different projects with independent environments), set `TEA_CODE_HOME`:

```sh
export TEA_CODE_HOME="$HOME/.config/tea-code"
```

Once set, **all** Tea Code data — config, sessions, logs, OAuth credentials, Tea Code-specific user Skills, global `AGENTS.md`, and more — lands under the new path. For the full reference on `TEA_CODE_HOME`, see [Environment variables](./env-vars.md).

::: tip Note

**Generic `.agents` resources** stay under the real OS home so they can be shared across tools. For example, user-level generic Skills remain at `~/.agents/skills/`, while Tea Code-specific user Skills move with `TEA_CODE_HOME` as `$TEA_CODE_HOME/skills/`.
:::
...
├── AGENTS.md               # Global Tea Code-specific agent instructions (optional)
├── mcp.json                # User-level MCP server declarations (optional)
├── skills/                 # Tea Code-specific user-level Skills (optional)
├── plugins/
...
- **`AGENTS.md`**: global Tea Code-specific agent instructions. This file moves with `TEA_CODE_HOME`; generic cross-tool instructions can still live under `~/.agents/AGENTS.md`.
...
- **`skills/`**: Tea Code-specific user-level Skills. This directory moves with `TEA_CODE_HOME`; generic cross-tool Skills can still live under `~/.agents/skills/`. See [Agent Skills](../customization/skills.md).
...
- **`cron/`**: scheduled task persistence; reloaded into the scheduler when the session is resumed with `tea-code --session`. See [Scheduled tasks](../reference/tools.md#scheduled-tasks).
...
When reporting a bug, prefer exporting the relevant session with `tea-code export` (see [tea-code command](../reference/kimi-command.md)); the session log is included in the export by default. Add `--no-include-global-log` if you do not want to share the global log.
...
Deleting the data root directory (`~/.tea-code/` or the path set by `TEA_CODE_HOME`) removes all runtime data. To clear only part of the data:

| Goal | Action |
| --- | --- |
| Reset configuration | Delete `~/.tea-code/config.toml` |
| Reset terminal UI preferences | Delete `~/.tea-code/tui.toml` |
| Clear all sessions | Delete `~/.tea-code/sessions/` and `session_index.jsonl` |
| Clear diagnostic logs | Delete `~/.tea-code/logs/` |
| Clear input history | Delete `~/.tea-code/user-history/` |
| Reset update state | Delete `~/.tea-code/updates/latest.json` |
| Force re-download of managed `rg` and `fd` | Delete `~/.tea-code/bin/` |
...
| Remove user-level MCP declarations | Delete `$TEA_CODE_HOME/mcp.json` (default `~/.tea-code/mcp.json`) |
| Clear global Tea Code-specific agent instructions | Delete `$TEA_CODE_HOME/AGENTS.md` (default `~/.tea-code/AGENTS.md`) |
| Clear plugin install records | Delete `$TEA_CODE_HOME/plugins/` (local plugin source directories are not affected) |
| Clear Tea Code-specific user-level Skills | Delete `$TEA_CODE_HOME/skills/` (default `~/.tea-code/skills/`) |

## Next steps

- [Configuration files](./config-files.md) — full reference for `config.toml` fields
- [Environment variables](./env-vars.md) — detailed usage of `TEA_CODE_HOME` and related path variables
