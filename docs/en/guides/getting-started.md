...
Install Tea Code through npm. Tea Code 0.4.0 corresponds to Kimi Code 0.41.0 and uses `TEA_CODE_HOME` or `~/.tea-code` for user state.

::: tip Before you install
Tea Code CLI is a fully interactive TUI application. For the best visual experience, run it in a terminal with true-color and ligature support, such as [Kitty](https://sw.kovidgoyal.net/kitty/) or [Ghostty](https://ghostty.org/).
:::
...
Move into your project directory and run `tea-code` to start the interactive UI:
...
```sh
tea-code -p "Take a look at this project's directory structure"
```
...
If you want to connect Anthropic, OpenAI, Google, or other providers, edit `~/.tea-code/config.toml` directly to configure the API key. See [Providers and models](../configuration/providers.md) for details. For the full reference of all config options, see [Configuration files](../configuration/config-files.md), [Environment variables](../configuration/env-vars.md), and [Configuration overrides](../configuration/overrides.md).
:::

## Your first conversation

Once logged in, describe a task in natural language. A good starting point is to let Tea Code CLI familiarize itself with the project:

```
Take a look at this project's directory structure and briefly describe what each directory is for.
```

Tea Code CLI automatically calls file-reading, search, and other tools to browse the relevant content before responding. Read-only operations are executed automatically by default without requiring confirmation. For operations that modify files or run shell commands, it asks for your confirmation before proceeding.
...
Tea Code CLI plans the steps, modifies the code, runs the tests, and tells you what it did at each step.
...
Tea Code CLI stores its local data under `~/.tea-code/` by default — config files, session records, logs, and the update cache. To move it elsewhere, point to a new path via the `TEA_CODE_HOME` environment variable. For the full directory layout, see [Data locations](../configuration/data-locations.md) and [Environment variables](../configuration/env-vars.md).
...
**Upgrade**: run `tea-code upgrade` — the CLI checks for the latest version and presents update options. Choose `Install update now` to upgrade based on your current install source. You can also upgrade directly via the package manager:

```sh
npm install -g @futuretea/tea-code@latest
```

**Uninstall**: remove the npm package:
