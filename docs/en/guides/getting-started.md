...
Install Tea Code globally with a JavaScript package manager.

::: tip Before you install
Tea Code is a fully interactive TUI application. For the best visual experience, run it in a terminal with true-color and ligature support, such as [Kitty](https://sw.kovidgoyal.net/kitty/) or [Ghostty](https://ghostty.org/).
:::
...
**Upgrade**: run `tea-code upgrade` — the CLI checks npm for the latest version and presents update options. You can also upgrade directly via the package manager:

```sh
npm install -g @futuretea/tea-code@latest
```

**Uninstall**: if you installed via npm:
...
Once logged in, describe a task in natural language. A good starting point is to let Tea Code familiarize itself with the project:

```
Take a look at this project's directory structure and briefly describe what each directory is for.
```

Tea Code automatically calls file-reading, search, and other tools to browse the relevant content before responding. Read-only operations are executed automatically by default without requiring confirmation. For operations that modify files or run shell commands, it asks for your confirmation before proceeding.
...
Tea Code plans the steps, modifies the code, runs the tests, and tells you what it did at each step.
