 ...
 *
 * Input dir must contain files matching: tea-code-<target>.zip.sha256
...
// Tag 格式 `@futuretea/tea-code@x.y.z` 或 `vx.y.z` 或 `x.y.z`，都归一化到 x.y.z
const version = tag.replace(/^@futuretea\/tea-code@/, '').replace(/^v/, '');

const entries = await readdir(inputDir);
const sumFiles = entries.filter((f) => /^tea-code-[a-z0-9-]+\.zip\.sha256$/.test(f));

if (sumFiles.length === 0) {
  console.error(`No tea-code-<target>.zip.sha256 files found in ${inputDir}`);
  process.exit(1);
}
...
for (const sumFile of sumFiles.sort()) {
  ...
  // tea-code-darwin-arm64.zip → darwin-arm64
  const target = filename.replace(/^tea-code-/, '').replace(/\.zip$/, '');
