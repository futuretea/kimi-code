...
const executableName = process.platform === 'win32' ? 'tea-code.exe' : 'tea-code';
...
describe('native release artifacts', () => {
  afterEach(() => {
    ...
    rmSync(resolve(artifactsDir, `tea-code-${target}.zip`), { force: true });
    rmSync(resolve(artifactsDir, `tea-code-${target}.zip.sha256`), { force: true });
  });
  ...
  it('packages the native binary as a zip archive and checksums the archive', async () => {
    ...
    const archivePath = resolve(artifactsDir, `tea-code-${target}.zip`);
    ...
    expect(readFileSync(checksumPath, 'utf-8')).toBe(
      `${sha256(readFileSync(archivePath))}  tea-code-${target}.zip\n`,
    );
  });

  it('produces a manifest from zip archive checksums', async () => {
    const releaseDir = await mkdtemp(join(tmpdir(), 'tea-code-manifest-zip-'));
    const archiveBytes = Buffer.from('fake zip bytes');
    const checksum = sha256(archiveBytes);
    await writeFile(join(releaseDir, 'tea-code-darwin-arm64.zip'), archiveBytes);
    await writeFile(
      join(releaseDir, 'tea-code-darwin-arm64.zip.sha256'),
      `${checksum}  tea-code-darwin-arm64.zip\n`,
    );

    await execFileAsync(process.execPath, [manifestScript, releaseDir, '@futuretea/tea-code@0.5.0']);
    ...
    expect(manifest).toEqual({
      version: '0.5.0',
      tag: '@futuretea/tea-code@0.5.0',
      platforms: {
        'darwin-arm64': {
          filename: 'tea-code-darwin-arm64.zip',
          checksum,
        },
      },
    });
