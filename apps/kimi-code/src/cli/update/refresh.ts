...
import { NPM_PACKAGE_NAME, type UpdateCache } from './types';
...
export async function fetchLatestFromNpmRegistry(): Promise<{
  ...
  if (typeof body.version !== 'string' || valid(body.version) === null) {
    throw new Error('npm registry returned invalid semver');
  }
