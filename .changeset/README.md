...
| --- | --- | --- |
| `@futuretea/tea-code` | `apps/kimi-code` | CLI / TUI application — provides the `tea-code` command after install |
...
| --- | --- |
| Only modifies TUI behavior in `@futuretea/tea-code` | Add `patch` / `minor` / `major` to `@futuretea/tea-code` |
| Only modifies internal packages, no user-visible change in SDK / CLI | Usually no changeset needed |
| Internal package fix changes the CLI user experience | Add a changeset to `@futuretea/tea-code` describing the user-visible fix |
| Internal package adds a new capability exposed by the SDK | Add a changeset to `@moonshot-ai/kimi-code-sdk` |
| SDK behavior change affects CLI user experience | Add changesets to both `@moonshot-ai/kimi-code-sdk` and `@futuretea/tea-code` |
| Provider abstraction change affects SDK / CLI | Add changesets to the affected `@moonshot-ai/kimi-code-sdk` and/or `@futuretea/tea-code` |
...
1. Open each publishable package's page on the npm website, e.g. `https://www.npmjs.com/package/@futuretea/tea-code`.
...
- Changes under `plugins/` (the bundled official plugins such as `kimi-datasource`) do **not** need a changeset: each plugin carries its own version in `kimi.plugin.json` and `plugins/marketplace.json` and is distributed via the marketplace CDN, separately from the `@futuretea/tea-code` npm package.
- Run `pnpm changeset version` and commit its generated version files to `tea/moonshot-038`; pushing that versioned commit triggers the Release workflow to publish directly.
- Do not create a release PR for `tea/moonshot-038`; this distribution publishes from the branch after its changesets have been versioned and committed.
- Do not add release changesets for private internal packages; only select `@futuretea/tea-code` and `@moonshot-ai/kimi-code-sdk`.
- If a change in an underlying internal package alters user-visible behavior or public API of a publishable package, add a changeset to the affected publishable package. For example, when a bug fixed in `@moonshot-ai/agent-core` resolves an issue CLI users encounter, add a changeset to `@futuretea/tea-code` describing the user-visible fix.
- `@futuretea/tea-code` is the official CLI package name; after a global install it provides the `tea-code` command.
