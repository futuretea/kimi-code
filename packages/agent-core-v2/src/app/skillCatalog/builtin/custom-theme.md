---
name: custom-theme
description: Create or edit a Tea Code custom color theme — a JSON file under the resolved TEA_CODE_HOME data directory that recolors the TUI. Use when the user wants their own theme, asks for a specific palette or mood, or wants to tweak an existing custom theme's colors.
---

# Create a Tea Code custom theme (custom-theme)

Help the user design, write, and apply a custom color theme for the Tea Code TUI. A theme is a single JSON file; the TUI ships with `dark`, `light`, and `auto`, and any file the user adds becomes selectable alongside them.
...
- **Never assume the data directory is `~/.tea-code`.** Always resolve `$TEA_CODE_HOME` first with the Bash command below.
...
The Tea Code runtime resolves the data directory as `TEA_CODE_HOME` first, falling back to `~/.tea-code`. Theme files live inside the `themes/` subdirectory of that data directory.

Before doing anything, resolve the actual data root with Bash so you don't write to the wrong place. Check whether `TEA_CODE_HOME` is set and fall back to `~/.tea-code` when it is empty:
...
Use the first line when it is non-empty; otherwise use the second line. In the rest of this skill, `<TEA_CODE_HOME>` means that resolved data root — **never assume `~/.tea-code`**. Theme files live at `<TEA_CODE_HOME>/themes/<name>.json`. Create the `themes/` directory if it doesn't exist.

## What a theme is

- A theme lives at `<TEA_CODE_HOME>/themes/<name>.json`.
...
2. **Resolve the actual theme directory and current theme(s).**
   - Resolve the data root by checking `echo "$TEA_CODE_HOME"`; if empty, use `echo "$HOME/.tea-code"`. Use `<root>/themes` for every subsequent step.
   - If tweaking an existing custom theme, **Read** `<TEA_CODE_HOME>/themes/<name>.json` first — never overwrite a theme you haven't read.
   - Starting fresh: build a `colors` object from the token table. You can `ls <TEA_CODE_HOME>/themes/` and Read one of the user's existing themes as a reference for the format.
...
4. **Create a candidate file; never edit the live theme in place.**
   - Use Bash to create a candidate. If the target theme already exists, copy it verbatim: `cp <name>.json <name>.json.new` (inside `<TEA_CODE_HOME>/themes/`). If it doesn't exist, use **Write** to create a minimal skeleton named `<name>.json.new`.
