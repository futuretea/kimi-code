...
The Tea Code runtime resolves the user-global directory as `TEA_CODE_HOME`
first, falling back to `~/.tea-code`. Before touching the user-global file,
resolve the actual directory with Bash so you don't read or write the wrong
one. Check whether `TEA_CODE_HOME` is set and fall back to `~/.tea-code`
when it is empty:
...
rest of this skill, `<TEA_CODE_HOME>` means that resolved data root —
**never assume `~/.tea-code`**.

- User-global: `<TEA_CODE_HOME>/mcp.json`. Use for servers you want
  everywhere.
...
- Project-local: `<cwd>/.kimi-code/mcp.json`. Use for Tea Code-specific
...
1. **Pick a scope.** Infer it from the user's words when you can
   ...
   current directory / Tea Code-specific / `.kimi-code` → project-local). When
...
3. **Write and tell them how to reload MCP servers.** Preserve unrelated
   ...
   restart `tea-code` for the change to take effect.
