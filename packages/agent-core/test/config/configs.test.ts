...
describe('config path env override', () => {
  it('uses TEA_CODE_HOME when no explicit homeDir is supplied and ignores the old env', () => {
    const savedTea = process.env['TEA_CODE_HOME'];
    const savedKimi = process.env['KIMI_CODE_HOME'];
    try {
      process.env['KIMI_CODE_HOME'] = '/tmp/old-kimi-from-env';
      process.env['TEA_CODE_HOME'] = '/tmp/tea-from-env';

      expect(resolveKimiHome()).toBe('/tmp/tea-from-env');
      expect(resolveKimiHome('/tmp/kimi-explicit')).toBe('/tmp/kimi-explicit');
      expect(resolveConfigPath({})).toBe('/tmp/tea-from-env/config.toml');
      expect(resolveConfigPath({ configPath: '/tmp/custom.toml' })).toBe('/tmp/custom.toml');
    } finally {
      if (savedTea === undefined) delete process.env['TEA_CODE_HOME'];
      else process.env['TEA_CODE_HOME'] = savedTea;
