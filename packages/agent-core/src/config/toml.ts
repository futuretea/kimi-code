  ...
  try {
    ...
    if (error instanceof KimiError && error.code === ErrorCodes.CONFIG_INVALID) {
      throw new KimiError(
        ...
        `Cannot change settings while ${filePath} is invalid — fix it first (run \`tea-code doctor\` for details).`,
        { cause: error },
      );
    }
    throw error;
    ...
    if (data !== undefined) {
      ...
      if (salvaged.config === undefined) {
        ...
        if (salvaged.dropped.length > 0) {
          fileWarnings.push(
            `Ignored invalid config in ${filePath}: ${salvaged.dropped.join(', ')}. Run \`tea-code doctor\` for details.`,
          );
        }
      }
    }
