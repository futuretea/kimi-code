...
[![License](https://img.shields.io/badge/license-MIT-blue)](LICENSE) [![Docs](https://img.shields.io/badge/docs-online-blue)](docs/zh/)

[Documentation](docs/zh/) · [Issues](https://github.com/futuretea/kimi-code/issues) · [English](README.md)


![Tea Code 的使用演示](./docs/media/intro.gif)


## 什么是 Tea Code

Tea Code 是一个运行在终端里的 AI 编程 agent，可以帮你读写代码、执行 shell 命令、检索文件、抓取网页，并根据反馈自主决定下一步动作。可对接 Kimi 模型和其他兼容厂商。

## 安装

使用 npm 安装，需要 Node.js 22.19.0 或更高版本。

```sh
npm install -g @futuretea/tea-code
```

> Windows 用户首次启动前还需要安装 [Git for Windows](https://gitforwindows.org/)，Tea Code 会使用其中的 Git Bash 作为 Shell 环境。如果 Git Bash 安装在非标准路径，请把 `KIMI_SHELL_PATH` 设为 `bash.exe` 的绝对路径。
...
npm 安装、升级、卸载方式，见[快速上手](docs/zh/guides/getting-started.md)。
...
首次启动时，在 Tea Code 里输入 `/login`，选择 Kimi OAuth 或 Moonshot AI Open Platform API 密钥登录。登录完成后，可以先让它熟悉项目：
...
- **npm 发行** 通过 `@futuretea/tea-code` 安装或升级。
...
Tea Code 支持 [Agent Client Protocol](https://agentclientprotocol.com/)，ACP 兼容的编辑器 / IDE（Zed、JetBrains……）可以通过 stdio 直接驱动会话。登录一次后，把编辑器指向 `tea-code acp` 子命令即可，无需重复登录。
...
随后在 Zed 的 Agent 面板新建对话即可。JetBrains 配置与排障见[在 IDE 中使用](docs/zh/guides/ides.md)，完整能力矩阵见 [`tea-code acp` 参考](docs/zh/reference/kimi-acp.md)。
